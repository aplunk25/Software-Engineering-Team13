...
while (True):
    ...
    UDPServerSocket.sendto(bytesToSend, address) #replace second part with address for UDP_client testing

    

    # Also broadcast to traffic generator (or anyone listening on 7500)

    UDPBroadcastSocket.sendto(message_bytes, ('127.0.0.1', broadcastPort)) #message_bytes to send exactly what the client sent
